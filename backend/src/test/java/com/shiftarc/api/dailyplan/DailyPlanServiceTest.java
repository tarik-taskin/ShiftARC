...
class DailyPlanServiceTest {
    ...
    void allocatesDailyTargetsByPriorityIntoMatchingBlocks() {
        ...
        when(repository.activeTasks(LocalWorkspace.ID)).thenReturn(List.of(
            new PlanningTask(UUID.randomUUID(), "WORK_ITEM", "Teslim", null, 5, 300, LocalDate.of(2026, 6, 28), null, null, 100, 0, Set.of(category)),
            new PlanningTask(UUID.randomUUID(), "HABIT", "İngilizce", null, 4, null, null, 360, null, 0, 60, Set.of(category))
        ));
    ...
    @Test
    void fillsRemainingCapacityWithOpportunitiesAfterRequiredTasks() {
        UUID category = UUID.randomUUID();
        UUID dayType = UUID.randomUUID();
        Clock clock = Clock.fixed(Instant.parse("2026-06-19T09:00:00Z"), ZoneOffset.UTC);
        DailyPlanService service = new DailyPlanService(repository, clock);
        ...
        when(repository.assignedDayType(LocalWorkspace.ID, LocalDate.of(2026, 6, 19))).thenReturn(new DayTypeSource(dayType, "İş Günü"));
        when(repository.sourceBlocks(dayType)).thenReturn(List.of(
            new SourceBlock(UUID.randomUUID(), "İş", 480, 540, Set.of(category))
        ));
        when(repository.activeTasks(LocalWorkspace.ID)).thenReturn(List.of(
            new PlanningTask(UUID.randomUUID(), "WORK_ITEM", "Teslim", null, 5, 20, LocalDate.of(2026, 6, 19), null, null, 0, 0, Set.of(category)),
            new PlanningTask(UUID.randomUUID(), "OPPORTUNITY", "Java derinleşme", "Core sorular", 4, null, null, null, 30, 0, 0, Set.of(category))
        ));
        when(repository.find(eq(LocalWorkspace.ID), any())).thenReturn(null, response(dayType));
        ...
        verify(repository).createPlan(any(), eq(LocalWorkspace.ID), eq(LocalDate.of(2026, 6, 19)), eq("UTC"), any(), blocks.capture(), any());
        org.assertj.core.api.Assertions.assertThat(blocks.getValue().get(0).items())
            .extracting(item -> item.taskTitle(), item -> item.taskStageTitle(), item -> item.endMinute() - item.startMinute())
            .containsExactly(
                org.assertj.core.groups.Tuple.tuple("Teslim", null, 20),
                org.assertj.core.groups.Tuple.tuple("Java derinleşme", "Core sorular", 30)
            );
    }
