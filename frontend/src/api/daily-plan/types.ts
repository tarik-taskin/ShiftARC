...
const itemSchema = z.object({
  id: z.string().uuid(), taskId: z.string().uuid(), taskTitle: z.string(), taskType: z.enum(['WORK_ITEM', 'HABIT']), importance: z.number().int(), plannedStartMinute: z.number().int(), plannedEndMinute: z.number().int(), status: z.enum(['PLANNED', 'ACTIVE', 'COMPLETED', 'SKIPPED']),
})
