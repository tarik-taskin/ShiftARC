...
const dayTypeSchema = z.object({ id: z.string().uuid(), name: z.string(), color: z.string() })
export const triggerSchema = z.object({
  ...
  dayTypes: z.array(dayTypeSchema),
  version: z.number().int().nonnegative(),
})
export const triggerListSchema = z.array(triggerSchema)
export type TriggerRule = z.infer<typeof triggerSchema>
export type TriggerInput = Pick<TriggerRule, 'type' | 'scheduleType' | 'title' | 'description' | 'importance' | 'durationMinutes' | 'intervalMinutes' | 'occurrenceTarget'> & { categoryIds: string[]; dayTypeIds: string[]; version?: number }
