...
export function TasksPage() {
  ...
  return <div className="space-y-6">
    ...
    <section aria-label="Görev listesi" className="overflow-hidden rounded-xl border bg-card">{tasks.data?.map((task) => <TaskRow key={task.id} task={task} pending={setTaskStatus.isPending} onEdit={() => openEdit(task)} onStatus={(nextStatus) => setTaskStatus.mutate({ id: task.id, version: task.version, status: nextStatus })} />)}</section>
