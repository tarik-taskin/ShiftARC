...
export function FocusPage() {
  ...
  const tasks = useTasks({ status: 'ACTIVE', sort: 'PRIORITY' }); const [taskId, setTaskId] = useState(''); const [now, setNow] = useState(() => Date.now()); const autoFinished = useRef<string | null>(null)
  const active = pomodoro.data?.activeSession
  const remaining = active ? Math.max(0, Math.ceil((new Date(active.plannedEndAt).getTime() - now) / 1000)) : 0
  ...
  if (pomodoro.isPending) return <p role="status">Odak yükleniyor…</p>
  if (pomodoro.isError) return <ErrorState title="Odak yüklenemedi" retry={() => pomodoro.refetch()} />
  return <div className="space-y-6"><PageHeader title="Odak sayacı" description="Tek bir işe yer aç. Odak ve molalarını buradan yönet." />
    {start.error || finish.error || update.error ? <p role="alert" className="rounded-lg border border-destructive/30 p-3 text-destructive">{(start.error ?? finish.error ?? update.error)?.message}</p> : null}

    <section className="grid gap-6 lg:grid-cols-[1.35fr_.65fr]"><div className="grid min-h-[360px] place-items-center rounded-xl border bg-card p-6 text-center"><div>{active ? <><div className="relative mx-auto grid size-64 place-items-center"><svg viewBox="0 0 100 100" className="absolute inset-0 size-full -rotate-90" aria-hidden="true"><circle cx="50" cy="50" r="46" fill="none" stroke="var(--muted)" strokeWidth="2" /><circle cx="50" cy="50" r="46" fill="none" stroke="var(--primary)" strokeWidth="2.5" strokeLinecap="round" pathLength="100" strokeDasharray="100" strokeDashoffset={100 - Math.min(100, remaining / (active.durationMinutes * 60) * 100)} /></svg><div className="relative px-5"><p className="text-xs font-semibold uppercase tracking-[.2em] text-muted-foreground">{label(active.phase)}</p><p role="timer" aria-live="off" className="mt-3 font-mono text-4xl font-medium">{minutes}:{seconds}</p><p className="mt-3 text-sm text-muted-foreground">{active.taskTitle ?? 'Serbest oturum'}</p></div></div><div className="mt-7 flex justify-center gap-3"><Button variant="outline" onClick={() => finish.mutate({ sessionId: active.id, version: active.version, cancelled: true })}><Square className="size-4" />İptal et</Button><Button onClick={() => finish.mutate({ sessionId: active.id, version: active.version })}><Check className="size-4" />Tamamla</Button></div></> : <><Target className="mx-auto size-12 text-primary" /><h2 className="mt-5 text-2xl font-semibold">Bir ritim seç</h2><label className="mx-auto mt-6 block max-w-sm text-left text-sm">Odak görevi<Select value={taskId} onValueChange={setTaskId} ariaLabel="Odak görevi" options={[{ value: '', label: 'Serbest odak' }, ...(tasks.data?.map((task) => ({ value: task.id, label: task.title })) ?? [])]} /></label><div className="mt-5 flex flex-wrap justify-center gap-2"><Button onClick={() => begin('FOCUS')}><Play className="size-4" />{settings?.focusMinutes ?? 25} dk odak</Button><Button variant="outline" onClick={() => begin('SHORT_BREAK')}><Coffee className="size-4" />Kısa mola</Button><Button variant="outline" onClick={() => begin('LONG_BREAK')}><Coffee className="size-4" />Uzun mola</Button></div></>}</div></div>
      <aside className="space-y-4"><div className="rounded-xl border bg-card p-5"><p className="text-sm text-muted-foreground">Bugünkü odak döngüsü</p><p className="mt-2 text-4xl font-semibold">{pomodoro.data?.focusCyclesToday ?? 0}</p><p className="mt-2 text-xs text-muted-foreground">{settings?.cyclesBeforeLongBreak ?? 4} döngüden sonra uzun mola önerilir.</p></div>{settings ? <SettingsForm settings={settings} pending={update.isPending} onSave={(value) => update.mutate(value)} /> : null}</aside></section>
    <section><h2 className="text-lg font-semibold">Son oturumlar</h2><div className="mt-3 grid gap-2">{pomodoro.data?.recentSessions.filter((s) => s.status !== 'ACTIVE').slice(0, 5).map((s) => <div key={s.id} className="flex items-center justify-between rounded-lg border bg-card px-4 py-3"><span>{label(s.phase)}{s.taskTitle ? ` · ${s.taskTitle}` : ''}</span><span className="text-sm text-muted-foreground">{s.durationMinutes} dk · {s.status === 'COMPLETED' ? 'Tamamlandı' : 'İptal'}</span></div>)}</div></section>
  </div>
}
